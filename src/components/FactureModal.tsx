...
import { X, FileText, Euro, Calendar, User, CreditCard } from 'lucide-react';
...
export const FactureModal: React.FC<FactureModalProps> = ({ onClose, patientId, consultationId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Si le montant restant est supérieur à 0, proposer le paiement immédiat
      if (newFacture.montant_restant > 0) {
        // Ajouter le nom du patient pour l'affichage dans le modal de paiement
        const factureWithPatientName = {
          ...newFacture,
          patient_name: patients.find(p => p.id === newFacture.patient_id)
            ? `${patients.find(p => p.id === newFacture.patient_id)?.prenom} ${patients.find(p => p.id === newFacture.patient_id)?.nom}`
            : undefined
        };
        
        // Stocker la facture créée pour le modal de paiement
        setFormData(factureWithPatientName);
        setShowPaymentModal(true);
      } else {
        // Fermer le modal si pas de paiement nécessaire
        onClose();
      }
  ...
  const handlePaymentComplete = () => {
    // Fermer le modal après paiement réussi
    onClose();
  };
  ...
  return (
    <>
      ...
      {showPaymentModal && 'id' in formData && (
        <StripePaymentModal
          facture={formData as FactureData}
